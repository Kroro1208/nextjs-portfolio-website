...
const ProjectCard = ({ project }) => {
    return (
        <Card className="group overflow-hidden relative xl:m-3">
                ...
                <div className="relative w-full h-[300px] flex items-center justify-center bg-tertiary
                 dark:bg-sky-200 xl:bg-yellow-100 overflow-hidden">
                    ...
                    <div className="flex gap-3">
                        ...
                        scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-300"
                        >
            ...
            <div className="h-full px-8 py-4 bg-sky-100 dark:text-gray-800 dark:bg-yellow-100">
                <Badge className="uppercase text-sm font-medium mb-2 absolute top-4 left-5">
                    {project.category}
                </Badge>
                <h4 className="h4 mb-1">{project.name}</h4>
                <p className="text-muted-foreground text-lg dark:text-gray-800">{project.description}</p>
            </div>
        </Card>
    )
}
